  ...
  <script>
    ...
    function initChart() {
      try {
        ...
        updateStatus('success', 'Chart ready - select a file to load data');
        return true;
      } catch (error) {
...
if __name__ == '__main__':
    ...
    print(f"   Load File:     POST http://{server_ip}:8000/api/load-file")
    print(f"   List Files:    GET  http://{server_ip}:8000/api/list-files")
    print(f"   Health Check:  GET  http://{server_ip}:8000/api/health")
    print(f"   Cache Info:    GET  http://{server_ip}:8000/api/cache-info")
    print("=" * 60)
    ...
    try:
        ...
        print("   1. Check if port 8000 is available: sudo netstat -tlnp | grep :8000")
        print("   2. Check firewall: sudo ufw allow 8000")
